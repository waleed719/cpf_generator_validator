...
// --- CPF Logic and Utility Functions ---
const formatCPF = (cpf) => {
  cpf = cpf.replace(/[^\d]/g, '');
...
const CPFValidatorGenerator = () => {
  ...
  const translations = {
    pt: {
      ...
      batchValIntro: 'Valide grandes listas de CPFs de uma vez. Cole uma lista, carregue um arquivo ou importe dados para verificar a validade de milhares de CPFs instantaneamente.',
      stats: {
      ...
      privacyText: 'A sua privacidade é a nossa prioridade. O CPF Tools não coleta, armazena ou transmite qualquer CPF inserido ou gerado. Todo o processamento, incluindo validação e geração, é realizado 100% no seu navegador (cliente-side), garantindo que seus dados permaneçam privados e seguros.',
      termsText: 'Ao utilizar o CPF Tools, você concorda que a ferramenta é fornecida "como está" (AS IS), sem garantias. É estritamente proibido o uso da ferramenta para qualquer finalidade ilegal, fraudulenta ou que viole as leis brasileiras.',
      legalText: 'AVISO: Os números de CPF gerados por esta ferramenta são matematicamente válidos, mas não correspondem a pessoas físicas reais. Eles servem unicamente como dados fictícios para fins de testes e desenvolvimento de sistemas.'
    },
    en: {
      home: 'Home',
      ...
      terms: 'Terms of Use',
      legal: 'Legal Notice',
      heroTitle: 'Professional CPF Validator & Generator - Free Online 2025',
      ...
      generatorBenefit1: 'Online Valid CPF Generation for system testing and automation.',
      generatorBenefit2: 'Region filter to generate state-specific CPFs.',
      generatorBenefit3: 'Format options (formatted and unformatted).',
      generatorBenefit4: '100% free, secure tool with unlimited use.',
      ...
      privacyText: 'Your privacy is our priority. CPF Tools does not collect, store, or transmit any entered or generated CPF. All processing is performed 100% in your browser (client-side), ensuring your data remains private and secure.',
      termsText: 'By using CPF Tools, you agree that the tool is provided "AS IS," without warranties. It is strictly forbidden to use the tool for any illegal, fraudulent purpose, or any activity that violates Brazilian laws.',
      legalText: 'NOTICE: The CPF numbers generated by this tool are mathematically valid but do not correspond to real individuals. They serve only as fictitious data for system testing and development purposes.'
    }
  };
  ...
  // Enhanced JSON-LD Schema with FAQPage and SoftwareApplication
  const getSchemaMarkup = () => {
      ...
      "@graph": [
        {
          ...
          "name": language === 'pt' ? "Gerador de CPF - CPF Tools" : "CPF Generator - CPF Tools",
          "alternateName": language === 'pt' ? "Gerador de CPF Válido Online" : "Valid CPF Generator Online",
          "description": t.heroSubtitle,
          ...
          "publisher": {
            ...
            "logo": {
              "@type": "ImageObject",
              "url": "https://geradorcpf.site/logo.png"
            }
          },
          "potentialAction": {
            "@type": "SearchAction",
        ...
        },
        {
          ...
          "name": "CPF Tools - Gerador e Validador",
    ...
    // Add FAQPage schema for home and about pages
    if (currentCleanPath === '/' || currentCleanPath === '/sobre-cpf-tools') {
      baseSchema["@graph"].push({
        "@type": "FAQPage",
        "mainEntity": [
          {
            "@type": "Question",
            "name": language === 'pt' ? "Como gerar um CPF válido?" : "How to generate a valid CPF?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": language === 'pt' 
                ? "Use nosso gerador de CPF online gratuito. Selecione a região desejada, escolha o formato (com ou sem pontuação) e clique em 'Gerar'. O algoritmo oficial brasileiro garante CPFs matematicamente válidos para testes."
                : "Use our free online CPF generator. Select the desired region, choose the format (with or without punctuation) and click 'Generate'. The official Brazilian algorithm guarantees mathematically valid CPFs for testing."
            }
          },
          {
            "@type": "Question",
            "name": language === 'pt' ? "Como validar se um CPF é válido?" : "How to validate if a CPF is valid?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": language === 'pt'
                ? "Digite ou cole o CPF no validador e clique em 'Validar'. Nossa ferramenta usa o algoritmo oficial de módulo 11 para verificar os dígitos verificadores instantaneamente, 100% no seu navegador."
                : "Enter or paste the CPF in the validator and click 'Validate'. Our tool uses the official modulo 11 algorithm to verify check digits instantly, 100% in your browser."
            }
          },
          {
            "@type": "Question",
            "name": language === 'pt' ? "O CPF gerado é real?" : "Is the generated CPF real?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": language === 'pt'
                ? "Não. Os CPFs gerados são matematicamente válidos (passam no algoritmo de validação), mas são fictícios e não correspondem a pessoas reais. São destinados exclusivamente para testes de software e desenvolvimento."
                : "No. Generated CPFs are mathematically valid (pass the validation algorithm), but are fictitious and do not correspond to real people. They are intended exclusively for software testing and development."
            }
          },
          {
            "@type": "Question",
            "name": language === 'pt' ? "A ferramenta é gratuita?" : "Is the tool free?",
            "acceptedAnswer": {
              "@type": "Answer",
              "text": language === 'pt'
                ? "Sim! O CPF Tools é 100% gratuito, sem limitações de uso, sem necessidade de registro e sem custos ocultos. Todo o processamento é feito localmente no seu navegador."
                : "Yes! CPF Tools is 100% free, with no usage limitations, no registration required, and no hidden costs. All processing is done locally in your browser."
            }
          }
        ]
      });
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="hidden md:flex items-center space-x-1">
              {navItems.map(item => (
                <NavLink key={item.id} path={item.path}>
                  <item.icon size={16} />
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50 font-sans">
      <Helmet>
        ...
        <meta property="og:site_name" content="CPF Tools" />
