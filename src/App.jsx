...
//     const meta = {
//       home: {
//         title: language === 'pt' ? 'Validador e Gerador de CPF Profissional - Gratuito e Online' : 'Professional CPF Validator & Generator - Free Online',
//         description: t.heroDescription,
//         keywords: language === 'pt' ? 'validador cpf, gerador cpf, cpf válido, validar cpf online, gerar cpf válido' : 'cpf validator, cpf generator, valid cpf, validate cpf online, generate valid cpf'
//       },
//       generator: {
//         title: t.generatorTitle,
//         description: t.generatorMetaDesc,
//         keywords: language === 'pt' ? 'gerador cpf, gerar cpf válido, cpf para teste, cpf fictício' : 'cpf generator, generate valid cpf, test cpf, fictitious cpf'
//       },
//       validator: {
//         title: t.validatorTitle,
//         description: t.validatorMetaDesc,
//         keywords: language === 'pt' ? 'validador cpf, validar cpf, verificar cpf, cpf válido' : 'cpf validator, validate cpf, verify cpf, valid cpf'
//       },
//       batch: {
...
//       }
//     };
//     return meta[currentPage] || meta.home;
//   };
...
//             <ul className="space-y-2">
//               {['about', 'contact', 'documentation', 'faq'].map((item, idx) => (
//                 <li key={item}>
//                   {idx < 2 ? (
//                     <button
...
//                     </button>
//                   ) : (
//                     <span className="text-sm sm:text-base text-gray-400">{t[item]}</span>
//                   )}
//                 </li>
//               ))}
//             </ul>
...
//               {['React 18', 'Tailwind CSS', 'PWA Ready', '100% Local'].map(tech => (
...
//               <span className="text-xs sm:text-sm text-gray-400">{language === 'pt' ? 'Privacidade' : 'Privacy'}</span>
//               <span className="text-xs sm:text-sm text-gray-400">{language === 'pt' ? 'Termos' : 'Terms'}</span>
//               <span className="text-xs sm:text-sm text-gray-400">{t.disclaimer}</span>
//             </div>
//           </div>
//         </div>
//       </div>
...
//         <link rel="canonical" href={`https://yourdomain.com/${currentPage === 'home' ? '' : currentPage}`} />
//         <link rel="alternate" hreflang="pt-BR" href="https://yourdomain.com/" />
...
const CPFValidatorGenerator = () => {
  ...
  const getCurrentPath = useCallback(() => {
    // Normalizes hash paths like '#/gerador' or '' to '/gerador' or '/'
    const hash = window.location.hash || '#/';
    return hash.substring(1).toLowerCase() || '/';
  }, []);

  const [currentHashPath, setCurrentHashPath] = useState(getCurrentPath());

  useEffect(() => {
    const handleHashChange = () => {
      setCurrentHashPath(getCurrentPath());
      window.scrollTo(0, 0); // Scroll to top on navigation
    };

    window.addEventListener('hashchange', handleHashChange);
    // Run once on load to ensure initial path is set correctly
    handleHashChange();
  ...
  const getPageIdFromPath = (path) => {
    ...
    if (path.startsWith('/gerador')) return 'generator';
    if (path.startsWith('/validador')) return 'validator';
    if (path.startsWith('/lote')) return 'batch';
    if (path.startsWith('/sobre')) return 'about';
    if (path.startsWith('/contato')) return 'contact';
    if (path.startsWith('/privacidade')) return 'privacy';
    if (path.startsWith('/termos')) return 'terms';
    if (path.startsWith('/aviso-legal')) return 'legal';
    return 'home';
  };

  const currentPageId = getPageIdFromPath(currentHashPath);
  // --- END HASH ROUTING LOGIC ---
  ...
  // --- SEO Meta Tags Logic (Using Hash Path) ---
  const getPageMeta = () => {
    const path = currentHashPath.split('/')[1] || 'home'; // 'gerador', 'validador', 'privacy', etc.
    const meta = {
      home: {
  ...
  const pageMeta = getPageMeta();
  const canonicalUrl = `https://geradorcpf.site/${currentHashPath.startsWith('/') ? currentHashPath.substring(1) : currentHashPath}`;

  // --- NAVIGATION (Refactored to use Hash Links) ---
  const Navigation = () => {
    const navItems = [
      ...
      <section className="bg-gradient-to-br from-blue-600 via-blue-700 to-blue-800 text-white py-16 sm:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-4xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <a
                href="#/gerador" // Refactored to hash link
                className="px-6 sm:px-8 py-3 sm:py-4 bg-white text-blue-600 rounded-lg font-bold text-base sm:text-lg hover:bg-blue-50 transition shadow-lg flex items-center justify-center gap-2"
              >
                {t.getStarted}
                <ChevronRight size={20} />
              ...
              <a
                href="#/sobre" // Refactored to hash link
                className="px-6 sm:px-8 py-3 sm:py-4 bg-transparent border-2 border-white text-white rounded-lg font-bold text-base sm:text-lg hover:bg-white hover:text-blue-600 transition"
              >
                {t.learnMore}
              </a>
      ...
      <section className="py-12 sm:py-20 bg-gradient-to-r from-blue-600 to-blue-800 text-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <a
              href="#/validador" // Refactored to hash link
              className="px-6 sm:px-8 py-3 sm:py-4 bg-transparent border-2 border-white text-white rounded-lg font-bold text-base sm:text-lg hover:bg-white hover:text-blue-600 transition"
            >
              {t.validateCPF}
            </a>
  ...
  const renderContent = () => {
    if (currentHashPath.startsWith('/gerador')) return <GeneratorPage />;
    if (currentHashPath.startsWith('/validador')) return <ValidatorPage />;
    if (currentHashPath.startsWith('/lote')) return <BatchToolsPage />;
    if (currentHashPath.startsWith('/sobre')) return <AboutPage />;
    if (currentHashPath.startsWith('/contato')) return <ContactPage />;
    if (currentHashPath.startsWith('/privacidade')) return <PrivacyPage />;
    if (currentHashPath.startsWith('/termos')) return <TermsPage />;
    if (currentHashPath.startsWith('/aviso-legal')) return <LegalNoticePage />;
    return <HomePage />;
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        <div className="border-t border-gray-700 pt-6 sm:pt-8">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4 text-center md:text-left">
            ...
            <div className="flex gap-4 sm:gap-6 flex-wrap justify-center">
              <a href="#/privacidade" className="text-xs sm:text-sm text-gray-400 hover:text-white transition">
                {language === 'pt' ? 'Privacidade' : 'Privacy'}
              </a>
              <a href="#/termos" className="text-xs sm:text-sm text-gray-400 hover:text-white transition">
                {language === 'pt' ? 'Termos' : 'Terms'}
              </a>
              <a href="#/aviso-legal" className="text-xs sm:text-sm text-gray-400 hover:text-white transition">
                {t.disclaimer}
              </a>
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50 font-inter">
      ...
      <React.Fragment>
        ...
        <meta property="og:description" content={pageMeta.description} />
        <meta property="og:type" content="website" />
        {/* Canonical URL updated to use the full path, essential for SEO with hash routing */}
